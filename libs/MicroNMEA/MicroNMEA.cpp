...
unsigned int MicroNMEA::parseUnsignedInt(const char *s, int len)
{
	int r = 0;
	while (len--)
		r = 10 * r + *s++ - '0';
...
long MicroNMEA::parseFloat(const char* s, int log10Multiplier, const char** eptr)
{
	int8_t neg = 1;
	long r = 0;
	while (isspace(*s))
...
long MicroNMEA::parseDegreeMinute(const char* s, int degWidth,
								  const char **eptr)
{
	if (*s == ',') {
		if (eptr)
...
MicroNMEA::MicroNMEA(void* buf, int len) :
	_badChecksumHandler(NULL),
	_unknownSentenceHandler(NULL)
{
	setBuffer(buf, len);
...
void MicroNMEA::setBuffer(void* buf, int len)
{
	_bufferLen = len;
	_buffer = (char*)buf;
	_ptr = _buffer;
